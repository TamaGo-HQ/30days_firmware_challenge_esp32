...
static void ultrason_task(void *arg) {
  sensor_msg_t msg;
  ...
  while (1) {
    ...
    if (!ultrason_read_data(ultrason_sensor, msg.data)) {
      ...
      vTaskDelay(pdMS_TO_TICKS(ULTRASON_SAMPLE_PERIOD_MS));
      continue;
    }

    // 2. Send to aggregator
    msg.type = SENSOR_ULTRASONIC;
    if (xQueueSend(ultrason_queue, &msg, 0) != pdTRUE) {
...
void ultrason_task_create(QueueHandle_t sensor_to_agg_q, UBaseType_t priority,
  ...
  xTaskCreate(ultrason_task, "ultrason_task", 2048, NULL, priority, NULL);
}
